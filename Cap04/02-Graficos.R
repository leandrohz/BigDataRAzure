...
# Demo
demo("graphics") #comandos para criar diferentes tipos de gráticos (Base Plot System)
...
#Geometria (cor, mudar cor do título, tamanho da fonte do título)
...
#gráfico boxplot (estatítico para comparar varíaveis em termos de média, mediana, 1º quartil, outlires)
...
# plot() - scatterplots #gráticos de dispersão
...
?par
par() #lista todos os parâmetros gráfico (inclusive default)
par('pch')
par('lty')

x = 2:4
plot(x, pch = "*") #tipo de ponto do gráfico
...
#cuidado com a resolução do monitor; possíveis erros (exemplo: figure margins too large)
legend("topright", pch = 1, col = c("blue", "red"), legend = c("Var1","Var2"))
par(mfrow = c(1,1))

#função par é global; seção do R studio;
...
plot(lynx, type="n", main="Type n") #limpar gráfico
...
# Plot #função para a escala onde é possível criar um gráfico de bolhas;
plot(expectativa ~ pibpercap, data = df_1982, log = "x",
